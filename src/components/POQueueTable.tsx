...
export const POQueueTable: React.FC<POQueueTableProps> = ({
  ...
  // Available filter options derived from all orders (including Failed)
  ...
  return (
    <div className="space-y-3.5">
      {/* 🔹 1. TOP HEADER WITH TITLE & ACTION BUTTONS (NO BOX BEHIND HEADER) */}
      <div className="flex flex-wrap items-end justify-between gap-4 py-0.5">
        <div>
        ...
        <div className="flex items-center">
          <button
            ...
            } font-semibold px-3 py-1.5 rounded text-xs flex items-center gap-1.5 transition-colors cursor-pointer shadow-xs`}
          >
      ...
      {/* 🔹 2. SEARCH BAR & SEARCHABLE MULTISELECT DROPDOWNS */}
      <div className="bg-white p-3.5 rounded-none border border-[#E0E0E0] shadow-2xs space-y-2.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              type="text"
              placeholder="Search by Customer Order No., Customer, Document, Uploader..."
              value={searchTerm}
              onChange={(e) => {
                setSearchTerm(e.target.value);
                setCurrentPage(1);
              }}
              className="w-full pl-9 pr-8 py-2 border border-[#E0E0E0] rounded-none text-xs bg-white text-[#262626] placeholder-gray-400 focus:outline-none focus:border-[#F8B800] transition-colors"
            />
            {searchTerm && (
              <button
                type="button"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2 border text-xs font-semibold rounded-none cursor-pointer transition-colors shadow-2xs whitespace-nowrap ${
              activeFilterCount > 0
                ? isThemeB
          ...
          {activeFilterCount > 0 && (
            <button
              ...
              className="px-2.5 py-2 text-xs font-semibold text-gray-500 hover:text-red-700 hover:bg-red-50 border border-gray-200 rounded-none cursor-pointer transition-colors whitespace-nowrap"
            >
              Clear All
            </button>
          )}
          ...
          <table className="w-full text-left text-[11.5px] border-collapse">
            <thead>
              ...
              >
                ...
                <th className={`${isThemeB ? 'py-1 px-2.5' : 'py-2 px-3'} text-right font-bold`}>Actions</th>
              </tr>
            </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`${isThemeB ? 'py-1 px-2.5' : 'py-2 px-3'} whitespace-nowrap`}>
                        <div className="font-mono font-bold text-[#4f4f4e] flex items-center gap-2 text-xs">
                      ...
                      <td className={`${isThemeB ? 'py-1 px-2.5' : 'py-2 px-3'} whitespace-nowrap`}>
                        <div
                          className="font-medium text-[#8f9494] truncate max-w-[200px] text-xs"
                          title={order.sourceFileName || 'Purchase_Order.pdf'}
                        >
                          {order.sourceFileName || 'Purchase_Order.pdf'}
                        </div>
                      </td>
                      ...
                      {/* 7. Actions */}
                      <td className={`${isThemeB ? 'py-1 px-2.5' : 'py-2 px-3'} text-right whitespace-nowrap`}>
                        <div className="flex items-center justify-end gap-1.5">
                          {/* Review Button - Not shown if order status is Failed */}
                          {isProcessing ? (
                            <button
                              disabled={true}
                            ...
                            /* No review button for Failed status as per user requirement */
                            null
                          ) : (
                            <button
                          ...
                          {onDeletePo && (
                            <button
                              onClick={() => onDeletePo(order.id)}
                              title="Delete Purchase Order"
                              className={`${
                                isThemeB ? 'w-6 h-6' : ''
                              } p-1 bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 rounded transition-colors cursor-pointer shadow-2xs flex items-center justify-center`}
                            >
      ...
      {isFilterModalOpen && (
        ...
        >
          ...
          >
            ...
            {/* Modal Body: 4 Searchable MultiSelect Dropdowns with light background & dark values */}
            <div className="p-6 space-y-4 text-xs bg-white">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* 3. Upload Date Selection (Specific Date or Date Range) */}
                <div>
